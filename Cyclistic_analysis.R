...
ggplot(data = trip_summary)+geom_col(mapping = aes(x=day_of_week,y=number_of_rides,fill = member_casual),position = 'dodge')+scale_y_continuous(labels = label_comma())+scale_fill_manual(values = c("member" = "#2c3e50","casual" = "#e74c3c"))+labs(title = "Total Ride Volume: Casual vs. Members",subtitle = "Casual riders surges on weekends,\nwhile annual members dominate weekday usage",x="Day of the Week",y="Total Number of Rides",fill = 'User Type')+theme_minimal()+theme(legend.position = "top")
